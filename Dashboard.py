...
def scrape_corona_data():
    ...
    #print(countries_data['Afghanistan'])
...
def plot_country_data(data):
    ...
    fig = px.scatter(df, x="attributs", y="values")
    return fig
def plot_country_continent_data(data):
    ...
    del res['Population']
    del res['1 Caseevery X ppl1 Deathevery X ppl1 Testevery X ppl']
    res.index.name = None
    frames = [res, country]
...
def init_figure():
    ...
    return plot_continent_data(data, keyword="New"),plot_country_data(data),plot_country_continent_data(data), plot_top_k_countries(10, "TotalCases"), plot_boxplots(data)
...
init_continent_fig, init_country_fig,init_country_continent_fig,init_k_countries_plot, init_box_fig, = init_figure()
...
app.layout = html.Div([
    ...
    html.Div([
       html.Br(),
        html.H2("Scatter to see the situation of Covid in Senegal", style={"text-align": "center"}),
        html.Br(),
    dcc.Graph(
    id='example-graph',
    figure=init_country_fig
    )
    ]),

    html.Div([
       html.Br(),
        html.H2("Bar to compare the deaths in Senegal to death of Covid in Africa", style={"text-align": "center"}),
        html.Br(),
    dcc.Graph(
    id='example-graph-2',
    figure=init_country_continent_fig
    )
    ]),
])
